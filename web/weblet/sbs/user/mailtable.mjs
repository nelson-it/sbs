...
class MneSbsUserMailTable extends MneDbTableView
{
  ...
  async ok()
  {
    var passwdweblet = await this.get_password();
    if ( ! passwdweblet.obj.run.passval ) return false;

    var newval = true;
    await this.execute_modified( async () => 
    {
      try { await MneDbView.prototype.ok.call(this, { passwd : passwdweblet.obj.run.passval }); } catch (e) { passwdweblet.obj.run.passval = undefined; newval = false };
    })
  ...
  getTableParamDel(p)
  {
    ...
    p.passwd = this.config.composeparent.obj.weblets[this.initpar.passwdweblet].obj.run.passval;
  ...
  async del()
  {
    ...
    var newval = true;
    try { await super.del(); } catch (e) { passwdweblet.obj.run.passval = undefined; newval = false };
    
    this.newvalues = newval;
  }
}

export default MneSbsUserMailTable
